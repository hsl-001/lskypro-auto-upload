...
export default class imageAutoUploadPlugin extends Plugin {
  ...
  //         imageList.map(image => {
  //           console.info('Processing image:', image);
  ...
  uploadAllFile() {
    ...
    this.helper.setValue(content);
    console.info(this.settings.concurrencyLimit);

    this.uploader.uploadFilesByPath(imageList.map(item => item.obspath),this.settings.concurrencyLimit).then(res => {
      if (res.success) {
        let uploadUrlList = res.result; // 只包含成功的上传
        const uploadUrlFullResultList = res.fullResult || [];
        ...
        // 过滤掉失败的文件，只保留上传成功的文件
        const successfulImages = imageList.filter((_, index) => uploadUrlFullResultList[index].success);
        ...
        if (this.settings.deleteSource) {
          console.info('Image List:', successfulImages); // 打印成功上传的图片列表
      ...
      this.app.workspace.on(
        ...
        (evt: ClipboardEvent, editor: Editor, markdownView: MarkdownView) => {
          ...
          if (this.settings.workOnNetWork) {
            ...
            if (imageList.length !== 0) {
              this.uploader
                .uploadFilesByPath(imageList.map(item => item.path),this.settings.concurrencyLimit)
                .then(res => {
                  let value = this.helper.getValue();
                  if (res.success) {
                    let uploadUrlList = res.result;

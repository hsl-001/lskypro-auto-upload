...
export interface PluginSettings {
  ...
  concurrencyLimit: number; // 添加并发上传限制
  [propName: string]: any;
}
  ...
  display(): void {
    ...
    if (this.plugin.settings.uploader === "LskyPro") {
      new Setting(containerEl)
        ...
        // Concurrency upload limit setting
        new Setting(containerEl)
        .setName("并发图片限制1-10")
        .setDesc("并发图片上传限制1-10,默认为5.")
        .addSlider(slider =>
          slider
            .setLimits(1, 10, 1) // 设置滑块范围，1-10
            .setValue(this.plugin.settings.concurrencyLimit || 5) // 默认值为5
            .onChange(async value => {
